...
public class CustomAccessTokenHandler extends AbstractHandler implements ManagedLifecycle {
    ...
    private String authorizationQueryParamName;
    ...
    public boolean handleRequest(MessageContext messageContext) {
        ...
        if (accessToken != null && !"".equals(accessToken)) {
            ...
            // swap authorization bearer header to use for the api authentication
            Map headers =
                    (Map) axis2MC.getProperty(org.apache.axis2.context.MessageContext.TRANSPORT_HEADERS);
            if (headers != null) {
                headers.put("Authentication", headers.get("Authorization"));
    ...
    private String removeTokenFromQueryParameters(String input, String accessToken) {
        input = input.replace(String.format("?%s=%s", authorizationQueryParamName, accessToken), "?");
        input = input.replace(String.format("&%s=%s", authorizationQueryParamName, accessToken), "");
        input = input.replace("?&", "?");
